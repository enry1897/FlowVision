...
try:
    ...
    while True:
        ...
        pose_results = pose.process(frames)

        # Disegna lo skeleton tracking e controlla il braccio destro
        if pose_results.pose_landmarks:
            mp.solutions.drawing_utils.draw_landmarks(
                frames, pose_results.pose_landmarks, mp_pose.POSE_CONNECTIONS)
            ...
            if check_hands_on_heart(pose_results.pose_landmarks.landmark, w, h):
                ...
                cv2.putText(frames, "Hands on Heart Detected!", (50, 50),
            ...
            if right_arm_high:
                ...
                cv2.putText(frames, "Right arm raised!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                right_arm_high = 1
        ...
        cv2.putText(frames, f"Level: {level}", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if level == max_level else (0, 0, 255), 2)

        if level == max_level:
            cv2.putText(frames, "Max Level Reached!", (50, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            # Mostra l'immagine risultante
        cv2.imshow("Hand and Body Tracking Detection", frames)
